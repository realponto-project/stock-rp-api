...
module.exports = class StatusExpeditionDomain {
  async add(bodyData, options = {}) {
    ...
    const response = await StatusExpedition.create(statusExpedition, { transaction })
  ...
  async update(bodyData, options = {}) {
    ...
    }
    {
      ...
      const statusExist = await StatusExpedition.findByPk(id, { transaction })
    ...
    if (!HasProp("status", newStatusExpedition) || !newStatusExpedition.status) {
      errors = true
      field.status = true
      message.status = "status cannot null"
    } else {
      const { status } = newStatusExpedition
    ...
    const oldStatusExpedition = await StatusExpedition.findByPk(bodyData.id, { transaction })
    ...
    const response = await StatusExpedition.findByPk(bodyData.id, { transaction })
  ...
  async delete(id, options = {}) {
    ...
    const statusExpedition = await StatusExpedition.findByPk(id, { transaction })
