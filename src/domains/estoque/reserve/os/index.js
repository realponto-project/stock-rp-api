...
module.exports = class OsDomain {
      ...
      const osPartsUpdatePromises = osParts.map(async (item) => {
        if (R.prop("id", item)) {
          ...
          if (technicianUpdated) {
            if (osPartsReturn.return !== "0"
            || osPartsReturn.output !== "0"
            || osPartsReturn.missOut !== "0") {
              throw new FieldValidationError([
                {
                  field: { message: true },
                  message: { message: "não é possivel editar a OS pois há itens que já foram liberados" }
                }
              ])
            }
            ...
            const technicianReserve = await TechnicianReserve.findOne({
              where: {
                ...
                data:
                  osPartsReturn.o.technicianId !== "0c451d60-f837-4a9e-b8a6-cab41a788133"
                    ? {
                      [Op.gte]: moment(osPartsReturn.o.date).startOf("day").toString(),
                      [Op.lte]: moment(osPartsReturn.o.date).endOf("day").toString()
                    }
            ...
            if (technicianReserve) {
              ...
              await Promise.all(equips.map(async (equip) => {
                await equip.update({ reserved: false, technicianReserveId: null }, { transaction })
              }))

              await technicianReserve.update(
                {
                  amount: technicianReserve.amount - parseInt(osPartsReturn.amount, 10),
                  amountAux: technicianReserve.amountAux - parseInt(osPartsReturn.amount, 10)
                },
                { transaction }
              )
          ...
          if (productBase.product.serial && productBase.product.category !== "peca".category) {
            const { serialNumberArray } = item

            if (serialNumberArray.length !== parseInt(item.amount, 10)) {
              errors = true
              field.serialNumbers = true
              message.serialNumbers = "quantidade adicionada não condiz com a quantidade de números de série."
            }
        ...
        const osPartsdeletePromises = osPartsAll.map(async (item) => {
          ...
          const technicianReserve = await TechnicianReserve.findOne({
            where: {
              ...
              data: osPartDelete.o.technicianId !== "0c451d60-f837-4a9e-b8a6-cab41a788133"
                ? {
                  [Op.gte]: moment(osPartDelete.o.date).startOf("day").toString(),
                  [Op.lte]: moment(osPartDelete.o.date).endOf("day").toString()
                }
                : { [Op.gte]: moment("01/01/2019").startOf("day").toString() }
            },
            transaction
  ...
  async getAll(options = {}) {
    ...
    const formatProduct = (productBases, index) => R.map(async (item) => {
      const { osParts } = item
      ...
      if (
        ...
        notDelet[index] = parseInt(amount, 10) !== item.osParts.serialNumbers.length
          || notDelet[index]
      }

      const quantMax = parseInt(amount, 10)
    ...
    const formatData = mapIndexed(async (item, index) => {
      const resp = {
        ...
        products: [
          ...(await Promise.all(formatProduct(item.productBases, index, item.os))),
    ...
    if (productBase) {
      const technicianReserve = await TechnicianReserve.findOne({
        where: {
          ...
          data: osPart.o.technicianId !== "0c451d60-f837-4a9e-b8a6-cab41a788133"
            ? {
    ...
    const formatedProducts = R.map(async (item) => {
      ...
      await Promise.all(
        item.productBases.map(async (productBase) => {
          const { osParts: { amount, missOut, output, return: retorno } } = productBase
          const equips = await Equip.findAll({
