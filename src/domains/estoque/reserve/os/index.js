...
module.exports = class OsDomain {
    ...
    if (reserveHasProp('osParts')) {
      ...
      if (osPartsAll.length > 0) {
        const osPartsdeletePromises = osPartsAll.map(async (item) => {
          ...
          console.log(JSON.parse(JSON.stringify(osPartDelete)))
  ...
  async output(bodyData, options = {}) {
    ...
    // const osPartsUpdate = await OsParts.findByPk(bodyData.osPartId, {
    //   transaction,
    // })
    ...
    const response = await OsParts.findByPk(bodyData.osPartId, {
      transaction,
    })
    // throw new FieldValidationError([{ field, message }]);
  ...
  async getAllOsPartsByParamsForReturn(options = {}) {
    ...
    const formatedProducts = R.map(async (item) => {
      ...
      await Promise.all(
        item.productBases.map(async (productBase) => {
          const {
            osParts: {
              amount, missOut, output, return: retorno,
            },
          } = productBase
          const equips = await Equip.findAll({
    ...
    return {
      // rows,
      rows: rows.filter(item => item.oId),
    }
  }
  ...
  async finalizarCheckout(bodyData, options = {}) {
    ...
    if (bodyDataNotHasProp('osPartId') || !bodyData.osPartId) {
      ...
      osPart = await OsParts.findByPk(bodyData.osPartId, {
        transaction,
      })
    ...
    if (bodyData.serial) {
      ...
      if (
        ...
        const {
          amount, return: retorno, missOut, output,
        } = osPart
